...
smooth_qr <- function(
  ...
  if (n_models != length(aheads)) {
    cli::cli_abort(
      "`length(aheads)` must be the same as the number of columns in `y`."
    )
  }
  ...
  if (degree == n_models) {
    cli::cli_warn(
      c(
        "The degree of smoothing is the same as the number of responses.",
    ...
    cli::cli_abort(
      c(
        "The degree of smoothing must be less than the number of responses.",
  ...
  if (length(bad_cols) > 0L) {
    ...
    if (is.null(nms)) {
      nms <- paste("col", bad_cols)
    }
    cli::cli_abort(
      c(
        "Cannot perform smooth quantile regression.",
  ...
  if (is.null(colnames(x))) {
    colnames(x) <- original_predictors
  }
  if (intercept) {
