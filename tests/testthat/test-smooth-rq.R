test_that("smoothrq handles the basics", {
  y <- matrix(rnorm(100), ncol = 4)
  x <- matrix(rnorm(125), ncol = 5)

  expect_warning(smooth_rq(x, y, degree = 4L))
  expect_error(smooth_rq(x, y, degree = 5L))

  expect_silent(smooth_rq(x, y))
